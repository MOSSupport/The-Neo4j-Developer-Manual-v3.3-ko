...
드라이버 인스턴스를 생성하려면 [연결 URI](./client-applications/#driver-connection-uris)와 [인증 정보](./client-applications/#driver-authentication)를 제공해야 합니다. 필요하다면 추가 설정 정보를 제공할 수도 있습니다. 이 모든 세부사항은 드라이버 생애동안 변경할 수 없습니다. 그러므로, (다양한 데이터 베이스와 작업하는 것과 같이) 다양한 설정을 할 때는 다양한 드라이버 객체를 사용해야 됩니다. 
